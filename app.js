...
const render = () => {
    ...
    for (let i = 0; i < points.length; i++) {
        if (points[i][1] < -200)
            rawColors.push(vec3(0.18039, 0.22353, 0.55686));
        else if (0.0 < points[i][1] && points[i][1] < 200.0)
            rawColors.push(vec3(0.14, 0.56, 0.31));
        else if (points[i][1] > 350.0) rawColors.push(vec3(1.0, 1.0, 1.0));
        else rawColors.push(vec3(0.24, 0.15, 0.08));
    }

    colors = [];
    for (var i = 0; i < rawColors.length; i += 3) {
        let c = getAvg(rawColors[i], rawColors[i + 1], rawColors[i + 2]);
        colors.push(c);
        colors.push(c);
        colors.push(c);
    }
...
const getHeight = (x, z) => noise.perlin2(-x / 0.5, -z / 1.5) * 1400; // perlin noise
